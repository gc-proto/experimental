...
#
# _data/tariff_tool_links.json is the routing table, and it is edited by hand by
# people who are not reading the template. These tests are the guardrail on
# that: a typo'd need, a renamed sector or a row that quietly routes nowhere
# should fail here, not go silently missing from the page.

require "minitest/autorun"
require_relative "support/expected"

class TestProgramData < Minitest::Test
  ...
  # The 0-based index into the JSON array — what a failure message needs to
  # point someone at the offending entry. Was a CSV line number (header, plus
  # 1-based) until the routing table moved to JSON.
  def entry_of(row)
    rows.index(row)
  end

  def test_the_data_loads_and_is_not_empty
    refute_empty rows, "no rows loaded from _data/tariff_tool_links.json"
    assert_equal Wizard.site_data["tariff_tool_links"].size, rows.size
  end

  def test_every_row_fills_its_required_fields
    missing = []
    rows.each do |r|
      REQUIRED.each do |col|
        ...
        missing << "entry #{entry_of(r)} (#{r['program_name']}): #{col} is blank"
      end
    end
  ...
  # the row is in the data file and on no page. Two values are not needs: "all" is a
  ...
  def test_need_values_are_in_the_vocabulary
    ...
    assert_empty bad.map { |r| "entry #{entry_of(r)}: need=#{r['need']} (known: #{known.join(', ')})" }.join("\n  "),
  ...
  def test_sector_values_are_in_the_vocabulary
    ...
    assert_empty bad.map { |r| "entry #{entry_of(r)}: sector=#{r['sector']}" },
  ...
  def test_region_values_are_in_the_vocabulary
    ...
    assert_empty bad.map { |r| "entry #{entry_of(r)}: region=#{r['region']}" },
  ...
  def test_statuses_are_from_the_known_set
    ...
    assert_empty bad.map { |r| "entry #{entry_of(r)}: status=#{r['status'].inspect}" },
  ...
  def test_route_markers_sit_where_they_can_be_read
    ...
    rows.select { |r| r["status"] == "route" }.each do |r|
      assert_equal "national", r["region"], "entry #{entry_of(r)}: a route marker outside the national column"
      assert_includes sectors, r["sector"], "entry #{entry_of(r)}: a route marker in a sector with no answer"
      assert_equal 1, Wizard::Expected.needs_of(r).size, "entry #{entry_of(r)}: a route marker for several needs at once"
    end
  end
  ...
  def test_urls_are_absolute_https
    ...
    rows.each do |r|
      ...
      %w[url_en url_fr].each do |col|
        v = r[col].to_s.strip
        next if v.empty?
        bad << "entry #{entry_of(r)}: #{col}=#{v}" unless v.start_with?("https://")
      end
    end
  ...
  def test_no_row_carries_a_stray_whitespace_key_value
    ...
    assert_empty bad.map { |r| "entry #{entry_of(r)}: #{r['program_name']}" },
  ...
  def test_every_rendered_row_has_a_french_name_and_url
    ...
    assert_empty gaps.map { |r| "entry #{entry_of(r)}: #{r['program_name']}" },
  ...
  def test_translated_rows_record_their_provenance
    ...
    assert_empty missing.map { |r| "entry #{entry_of(r)}: #{r['program_name']}" },
  ...
  # NRCan programs. The data file marks the BDC row `disputed` and four NRCan
  # programs took its place. Un-excluding that row would not fail any of the
  # data-driven tests above — they follow the data — so the decision is pinned
  ...
  def test_forestry_transformation_routes_to_nrcan_not_bdc
    ...
    assert_empty bdc.map { |r| "entry #{entry_of(r)}: #{r['program_name']}" },
  ...
  # of this page asked "Ontario" once and mapped it to both region values at
  ...
  # stops someone re-merging the two answers in the YAML, since the data rows
  ...
  def test_ontario_is_two_region_answers_not_one
    ...
    on.each do |r|
      refute_includes r["csv"], ";", "#{r['marker']} maps to more than one region value: #{r['csv']}"
    end
  ...
  def test_every_region_has_its_own_regional_program
    Wizard.text("en")["regions"].each do |reg|
      reg["csv"].split(";").each do |region_value|
        has_program = Wizard::Expected.live.any? { |r| r["region"] == region_value }
        assert has_program, "#{region_value} (#{reg['marker']}) has no program routed to it"
      end
    end
  end

  # Two rows pointing at the same page must be triaged, not shipped silently:
  # the data file has a `duplicate-url` status for exactly that. This forces a new
  ...
  def test_shared_destinations_are_flagged_as_duplicates
    ...
    Wizard::Expected.live.reject { |r| r["url_en"].to_s.strip.empty? }
      ...
      unflagged << "#{url}\n      " + plain.map { |r| "entry #{entry_of(r)} [#{r['status']}] #{r['program_name']}" }.join("\n      ")
    end
  ...
  def test_no_duplicate_rows
    ...
    assert_empty dups.keys.map { |k| k.join(" / ") }, "identical rows in the routing table"
  end
end
